...
<head>
    ...
    <title>Empresas - Fendipetroleo Nacional</title>
...
<script type="text/javascript">
    function confirmar() {
        var respuesta = confirm("¿Está seguro de que desea eliminar?\n Se eliminaran las ofertas y postulaciones de los usuarios")
        if (respuesta === true) {
            return true;
        } else {
            return false;
    ...
    <div>
        <div class="container pd">
            <div class="col-md-12 text-center">
                <form action="<?php echo constant('URL'); ?>crudEmpresas/buscar" method="POST">
                    <div class="row">
                        <div class="col-md-8">
            ...
            <table class="table table-bordered" style="margin-top:2%">
                <thead>
                    <tr>
                        ...
                        <th style="width:30%" scope="col">Contacto</th>
                ...
                <tbody>
                    <?php
                    include_once 'models/entity/empresa.php';
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php echo $empresa->nombreE . ' ' . $empresa->apellidoE; ?></td>
                            <td><?php if($empresa->estado == 1){ echo 'Por evaluar';}else{ echo 'Activo';} ?></td>
                            <td class="text-center">
                                <a class="btn btn-sm btn-info" href="<?php echo constant('URL') . 'verEmpresa/detalles/' . $empresa->id; ?>">
                                    <ion-icon style="font-size:20px" name="eye"></ion-icon>
                                </a>
                                <a class="btn btn-sm btn-primary" href="<?php echo constant('URL') . 'editarEmpresa/detalles/'. $empresa->id; ?>">
                                    <ion-icon style="font-size:20px" name="build-outline"></ion-icon>
                                </a>
                                <a class="btn btn-sm btn-danger" href="<?php echo constant('URL') . 'crudEmpresas/eliminarEmpresa/' . $empresa->id; ?>" onclick="return confirmar()">
                ...
                <ul class="pagination justify-content-center">
                    <?php if ($this->numPaginas != "0") { ?>
                        ...
                        <li class="page-item <?php echo  $_SESSION['pagina'] <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="<?php echo constant('URL'); ?>crudEmpresas/nuevaPagina/<?php echo $_SESSION['pagina'] - 1; ?>">Anterior</a>
                        </li>
                        <?php for ($i = 0; $i < $this->numPaginas; $i++) { ?>
                            <li class="page-item <?php echo $_SESSION['pagina'] == $i + 1 ? 'active' : '' ?>">
                                <a class="page-link" href="<?php echo constant('URL'); ?>crudEmpresas/nuevaPagina/<?php echo $i + 1; ?>"><?php echo $i + 1; ?></a>
                            </li>
                        <?php } ?>
                        <li class="page-item <?php echo  $_SESSION['pagina'] >= $this->numPaginas ? 'disabled' : '' ?>">
                            <a class="page-link" href="<?php echo constant('URL'); ?>crudEmpresas/nuevaPagina/<?php echo $_SESSION['pagina'] + 1; ?>">Siguiente</a>
                        </li>
                    <?php } ?>
                </ul>
            </nav>
